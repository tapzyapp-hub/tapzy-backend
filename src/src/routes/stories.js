...
router.get("/stories/feed/items", async (req, res) => {
  try {
    const currentProfile = req.currentProfile || null;
    const now = new Date();
    const excludeOwn = String(req.query.excludeOwn || "") === "1";
    const where = {
      expiresAt: { gt: now },
      mediaUrl: { not: null },
    };
    if (excludeOwn && currentProfile) {
      where.profileId = { not: currentProfile.id };
    }
    ...
    const items = stories
      ...
      .map((story) => {
        ...
        const displayName = profile.name || profile.username || "Tapzy Network";
        return {
          id: story.id,
