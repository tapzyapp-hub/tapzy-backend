    ...
    <head>
      ...
      <style>
        ...
        *{box-sizing:border-box}
        ...
        .sf-loader{position:fixed;z-index:999;inset:0;display:grid;place-items:center;background:radial-gradient(circle at 50% 42%,rgba(255,255,255,.045),transparent 28%),linear-gradient(180deg,#050609,#000 58%,#020305);overflow:hidden;transition:opacity .62s ease,visibility .62s ease;will-change:opacity}
        ...
        .sf-loader-logo{position:relative;z-index:1;display:flex;align-items:center;justify-content:center;gap:18px;filter:drop-shadow(0 18px 46px rgba(255,255,255,.16));animation:sfLoaderPulse 1.9s cubic-bezier(.45,0,.2,1) infinite;will-change:transform,filter,opacity}
        .sf-loader-logo::before{content:"";position:absolute;left:50%;top:50%;width:250px;height:140px;border-radius:999px;background:radial-gradient(circle,rgba(255,255,255,.16),rgba(255,255,255,0) 68%);transform:translate(-50%,-50%);filter:blur(14px);opacity:.72;animation:sfLoaderHalo 1.9s ease-out infinite;z-index:-1}
        .sf-loader-mark{width:116px;height:auto;object-fit:contain;filter:brightness(1.08) contrast(1.05)}
        .sf-loader-word{font-size:clamp(54px,15vw,92px);font-weight:950;letter-spacing:-.08em;line-height:.9;color:#fff;text-shadow:0 0 1px rgba(255,255,255,.8),0 18px 42px rgba(255,255,255,.13)}
        @keyframes sfLoaderPulse{0%,100%{transform:scale(1);opacity:.92;filter:drop-shadow(0 18px 46px rgba(255,255,255,.14))}50%{transform:scale(1.055);opacity:1;filter:drop-shadow(0 24px 68px rgba(255,255,255,.24))}}
