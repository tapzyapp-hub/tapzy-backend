...
function storyTrayCard(profile, story, isOwner) {
  ...
  const mediaHtml = hasMedia
    ...
    : `<div class="profile-story-card-textonly">${escapeHtml(story.text || (profile.name || profile.username || "Tapzy"))}</div>`;
...
function postPreviewCard(post) {
  ...
  const mediaHtml = hasMedia
    ? isVideo
      ? `<video class="profile-post-card-media" src="${escapeHtml(post.mediaUrl)}" muted playsinline preload="metadata"></video>`
      : `<img class="profile-post-card-media" src="${escapeHtml(post.mediaUrl)}" alt="Post media" />`
    : `<div class="profile-post-card-fallback">${escapeHtml(post.caption || "Tapzy Post")}</div>`;

  const eventPill = post.event
    ? `<div class="profile-post-event-pill">${escapeHtml(post.event.title || "Event")}</div>`
    : "";
  ...
  return `
    <div class="profile-post-card">
      ...
      <div class="profile-post-body">
        <div class="profile-post-date">${escapeHtml(new Date(post.createdAt).toLocaleDateString())}</div>
        ${
          post.caption
            ? `<div class="profile-post-caption">${escapeHtml(post.caption)}</div>`
            : `<div class="profile-post-caption profile-post-caption-muted">No caption</div>`
        }
      </div>
    </div>
  `;
...
router.get("/u/:username", async (req, res) => {
  try {
    ...
    const [activeStories, recentPosts, attendingEvent] = await Promise.all([
            ...
            <section class="profile-panel profile-attending-banner" style="margin-top:18px;">
              ...
              <div class="profile-attending-sub">
                ${attendingEvent.event.city ? `${escapeHtml(attendingEvent.event.city)} • ` : ""}${
                  attendingEvent.event.startAt ? escapeHtml(new Date(attendingEvent.event.startAt).toLocaleString()) : "Upcoming"
                }
              </div>
            ...
            <section class="profile-panel" style="margin-top:18px;">
              <div class="profile-panel-row">
                <div>
                  ...
                  <div class="profile-panel-subheading">Recent posts from this profile.</div>
                </div>

                <a class="profile-mini-action" href="/posts">Open Posts</a>
              </div>

              <div class="profile-posts-grid">
                ${recentPosts.map((post) => postPreviewCard(post)).join("")}
              </div>
            </section>
          `
          : isOwner
            ? `
              <section class="profile-panel" style="margin-top:18px;">
                <div class="profile-panel-row">
                  <div>
                    <h3 class="profile-panel-heading">Posts</h3>
                    <div class="profile-panel-subheading">No posts yet.</div>
                  </div>

                  <a class="profile-mini-action" href="/posts">Create Post</a>
                </div>
              </section>
            `
            : ""
    ...
    <style>
      ...
      .profile-post-media-wrap{
        position:relative;
        height:260px;
        overflow:hidden;
      }

      .profile-post-card-media{
        width:100%;
        height:100%;
        object-fit:cover;
        display:block;
      ...
      .profile-post-card-fallback{
        ...
        font-weight:900;
        background:
          radial-gradient(420px 180px at 50% 0%, rgba(95,182,255,.12), transparent 42%),
          linear-gradient(180deg, rgba(14,18,28,.96), rgba(8,10,16,.99));
      }
      ...
      .profile-post-overlay{
        position:absolute;
        inset:0;
        background:linear-gradient(180deg, rgba(0,0,0,.04), rgba(0,0,0,.38) 70%, rgba(0,0,0,.65));
        pointer-events:none;
      }

      .profile-post-event-pill{
        position:absolute;
        left:14px;
        top:14px;
        z-index:2;
