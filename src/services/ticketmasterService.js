...
function buildUrl(params = {}) {
  ...
  url.searchParams.set("size", String(params.size || 200));
...
function categoryFromItem(item) {
  ...
  const type = text(first?.type?.name);
  const subType = text(first?.subType?.name);

  return [genre, subGenre, type, subType, segment].find(Boolean) || "Event";
}
