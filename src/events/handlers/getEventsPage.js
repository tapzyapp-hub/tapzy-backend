...
function renderCleanStyles() {
  return [
    ...
    ".clean-events-page{position:relative;min-height:100vh;padding:calc(env(safe-area-inset-top,0px) + 14px) 16px calc(116px + env(safe-area-inset-bottom,0px));color:#fff;background:radial-gradient(circle at 50% -8%,rgba(48,124,255,.34),transparent 30%),radial-gradient(circle at 10% 14%,rgba(46,140,255,.16),transparent 28%),linear-gradient(180deg,#03070d 0%,#02050a 48%,#000 100%);isolation:isolate;}",
    ".clean-events-page::before{content:\"\";position:fixed;inset:0;z-index:-1;background:linear-gradient(180deg,rgba(29,87,175,.18),transparent 24%),radial-gradient(circle at 50% 34%,rgba(47,123,255,.08),transparent 36%);pointer-events:none;}",
    ".clean-events-page::after{content:\"\";position:fixed;inset:0;z-index:-1;background-image:linear-gradient(rgba(255,255,255,.025) 1px,transparent 1px);background-size:100% 44px;mask-image:linear-gradient(180deg,transparent,black 18%,transparent 82%);pointer-events:none;}",
    ".premium-events-top{max-width:960px;margin:0 auto 14px;display:flex;align-items:center;gap:13px;padding:3px 2px 5px;}",
    ".premium-events-mark{width:52px;height:52px;border:1px solid rgba(151,211,255,.28);border-radius:18px;display:grid;place-items:center;background:linear-gradient(145deg,#2f7bff,#1455df);box-shadow:0 0 34px rgba(47,123,255,.44),0 18px 48px rgba(0,0,0,.52),inset 0 1px 0 rgba(255,255,255,.25);cursor:pointer;}",
    ...
    ".premium-events-top p{margin:0 0 4px;color:rgba(177,214,255,.70);font-size:11px;font-weight:950;letter-spacing:2.7px;text-transform:uppercase;}",
    ".premium-events-top h1{margin:0;font-size:clamp(26px,7vw,42px);line-height:.94;font-weight:950;letter-spacing:0;color:#fff;text-shadow:0 0 34px rgba(62,142,255,.24);}",
    ".clean-events-filters{position:sticky;top:0;z-index:20;max-width:960px;margin:0 auto 12px;display:flex;gap:9px;overflow-x:auto;padding:8px 0 10px;scrollbar-width:none;background:linear-gradient(180deg,rgba(3,7,13,.92),rgba(3,7,13,.62),transparent);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);}",
    ".clean-events-filters::-webkit-scrollbar{display:none;}",
    ".clean-events-filter{flex:0 0 auto;display:inline-flex;align-items:center;gap:7px;min-height:42px;padding:0 16px;border-radius:999px;border:1px solid rgba(137,205,255,.18);background:linear-gradient(180deg,rgba(255,255,255,.070),rgba(255,255,255,.032));color:rgba(245,249,255,.88);text-decoration:none;font-size:13px;font-weight:950;backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);box-shadow:0 12px 34px rgba(0,0,0,.30),inset 0 1px 0 rgba(255,255,255,.08);}",
    ...
    ".premium-events-count{max-width:960px;margin:0 auto 13px;color:rgba(209,224,248,.48);font-size:12px;font-weight:850;letter-spacing:.8px;text-transform:uppercase;}",
    ".premium-events-list{max-width:960px;margin:0 auto;display:grid;gap:12px;}",
    ".premium-event-row{position:relative;min-height:104px;display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:12px;padding:15px 18px 15px 15px;border:1px solid rgba(116,198,255,.16);border-radius:28px;background:linear-gradient(180deg,rgba(8,14,24,.88),rgba(2,5,10,.84));box-shadow:0 22px 72px rgba(0,0,0,.44),0 0 0 1px rgba(55,137,255,.05),inset 0 1px 0 rgba(255,255,255,.075);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);overflow:hidden;transform:translateZ(0);}",
    ".premium-event-row::before{content:\"\";position:absolute;inset:-1px;border-radius:inherit;background:radial-gradient(circle at 12% 20%,rgba(47,123,255,.28),transparent 34%),linear-gradient(135deg,rgba(255,255,255,.10),transparent 26%,rgba(77,156,255,.06) 70%,transparent);opacity:.9;pointer-events:none;}",
    ...
    ".premium-event-rank{width:58px;height:58px;border-radius:20px;display:grid;place-items:center;background:linear-gradient(145deg,rgba(47,123,255,.90),rgba(18,75,173,.82));color:#e3efff;box-shadow:0 0 28px rgba(47,123,255,.24),0 10px 28px rgba(0,0,0,.32),inset 0 1px 0 rgba(255,255,255,.16);}",
    ...
    ".premium-event-eyebrow em,.premium-event-eyebrow i{font-style:normal;min-width:0;max-width:48%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:rgba(145,190,255,.76);font-size:10px;font-weight:950;letter-spacing:1.4px;text-transform:uppercase;}",
    ...
    ".premium-event-copy small{display:block;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:rgba(214,226,246,.58);font-size:clamp(14px,2vw,21px);line-height:1.15;font-weight:750;}",
    ...
    "@media(max-width:720px){.clean-events-page{padding-left:18px;padding-right:18px}.premium-events-top{margin-bottom:14px}.premium-events-mark{width:54px;height:54px;border-radius:18px}.premium-events-top h1{font-size:36px}.clean-events-filters{gap:10px;margin-bottom:10px}.clean-events-filter{min-height:44px;padding:0 17px;font-size:14px}.premium-events-count{margin-bottom:14px}.premium-events-list{gap:13px}.premium-event-row{min-height:102px;padding:15px 15px;border-radius:27px;grid-template-columns:1fr}.premium-event-main{grid-template-columns:66px minmax(0,1fr);gap:15px}.premium-event-rank{width:58px;height:58px;border-radius:19px}.premium-event-side{display:none}.premium-event-copy strong{font-size:27px}.premium-event-copy small{font-size:18px}}",
    "@media(max-width:430px){.clean-events-page{padding-left:18px;padding-right:18px}.premium-events-top{gap:13px}.premium-events-mark{width:52px;height:52px}.premium-events-top p{font-size:11px}.premium-events-top h1{font-size:34px}.premium-event-main{grid-template-columns:62px minmax(0,1fr);gap:14px}.premium-event-rank{width:56px;height:56px;border-radius:18px}.premium-event-rank b{font-size:19px}.premium-event-copy strong{font-size:23px}.premium-event-copy small{font-size:16px}.premium-event-row{min-height:98px;border-radius:25px;padding:15px 14px}.clean-events-filter{font-size:14px;padding:0 17px}}",
    "@media(max-width:370px){.premium-event-main{grid-template-columns:54px minmax(0,1fr);gap:12px}.premium-event-rank{width:50px;height:50px;border-radius:16px}.premium-event-rank b{font-size:18px}.premium-event-copy strong{font-size:20px}.premium-event-copy small{font-size:14px}.premium-event-row{border-radius:22px}.premium-events-top h1{font-size:31px}}",
    "</style>",
  ].join("");
}
