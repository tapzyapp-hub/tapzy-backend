...
function renderFeedVideo(url, className) {
  return `<video class="${className}" src="${escapeHtml(url)}" autoplay muted loop playsinline webkit-playsinline preload="auto" crossorigin="anonymous" data-keep-video-live="1" data-remove-if-blank="1"></video>`;
}
