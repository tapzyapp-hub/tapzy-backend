...
<style data-tapzy-assistant>
  ...
  .tz-ai-body{position:absolute;left:14px;right:14px;bottom:12px;z-index:6;display:flex;flex-direction:column;gap:12px;width:auto;max-height:44%;min-height:0;overflow:auto;padding:0;box-sizing:border-box;scrollbar-width:none}.tz-ai-body::-webkit-scrollbar{display:none}
  .tz-ai-msg{max-width:88%;min-width:0;margin:0;padding:11px 14px;border-radius:21px;font:510 12.25px/1.42 Inter,"SF Pro Text","Segoe UI",system-ui,sans-serif;letter-spacing:.03px;white-space:pre-wrap;overflow-wrap:anywhere;word-break:normal;background:linear-gradient(180deg,rgba(18,24,38,.9),rgba(9,14,24,.82));border:1px solid rgba(166,205,255,.2);box-shadow:0 18px 42px rgba(0,0,0,.34),inset 0 1px 0 rgba(255,255,255,.09),inset 0 -18px 44px rgba(44,105,190,.05);text-shadow:0 1px 8px rgba(156,206,255,.05);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);-webkit-font-smoothing:antialiased;text-rendering:geometricPrecision}.tz-ai-msg.bot{align-self:flex-start;width:100%;max-width:100%;color:rgba(249,252,255,.98);border-radius:21px}.tz-ai-msg.user{align-self:flex-end;width:auto;max-width:78%;font-weight:700;line-height:1.32;background:linear-gradient(145deg,#2f7bff,#1559e6);border-color:rgba(150,205,255,.24);color:#fff;box-shadow:0 14px 32px rgba(35,105,255,.26)}
  ...
  .tz-ai-debug{position:relative;z-index:5;margin:-4px 22px 8px;padding:8px 10px;border-radius:14px;background:rgba(8,18,34,.54);border:1px solid rgba(115,185,255,.16);color:rgba(186,219,255,.82);font-size:11px;font-weight:850;line-height:1.25;display:none}.tz-ai-debug.is-on{display:block}.tz-ai-debug strong{color:#fff}
...
<div class="tz-ai-root" data-tapzy-assistant data-username="__USERNAME__" data-page-type="__PAGETYPE__" data-auth-page="__ISAUTH__">
  ...
  <section class="tz-ai-panel" aria-label="Ask Tapzy assistant" role="dialog" aria-modal="false">
    ...
    <div class="tz-ai-debug" aria-live="polite"></div>
...
(function(){
  ...
  roots.forEach(function(root){
    ...
    var launch=root.querySelector('.tz-ai-launch'),panel=root.querySelector('.tz-ai-panel'),close=root.querySelector('.tz-ai-close'),body=root.querySelector('.tz-ai-body'),form=root.querySelector('.tz-ai-form'),input=root.querySelector('.tz-ai-input'),status=root.querySelector('.tz-ai-status'),debug=root.querySelector('.tz-ai-debug'),mic=root.querySelector('.tz-ai-mic'),send=root.querySelector('.tz-ai-send'),stateText=root.querySelector('.tz-ai-state-text');
    var memory=[],geo=null,geoAsked=false,busy=false,stateTimer=null,pillTimer=null,hiddenTapCount=0,speechUnlocked=false,ttsAudio=null,ttsUrl='',browserTimeZone=(function(){try{return Intl.DateTimeFormat().resolvedOptions().timeZone||'';}catch(_){return '';}})(),brainSessionId=(function(){try{var key='tapzyBrainSessionId';var existing=localStorage.getItem(key);if(existing)return existing;var id='browser:'+((typeof crypto!=='undefined'&&crypto.randomUUID)?crypto.randomUUID():String(Date.now())+'-'+Math.random().toString(36).slice(2));localStorage.setItem(key,id);return id;}catch(_){return ''}})();
    ...
    function shut(){stopTtsAudio();try{if(window.speechSynthesis)window.speechSynthesis.cancel();}catch(_){}panel.classList.remove('is-open');window.clearTimeout(pillTimer);}
    ...
    function beautify(text){var value=String(text||'');value=value.replace(/\\*\\*([^*]+)\\*\\*/g,'$1').replace(/\\*\\*/g,'').replace(/__([^_]+)__/g,'$1').replace(/_/g,' ').replace(new RegExp('\\\\x60([^\\\\x60]+)\\\\x60','g'),'$1').replace(/\\[([^\\]]+)\\]\\([^)]*\\)/g,'$1').replace(/\\(\\[\\]\\(|\\[\\]\\(|\\(\\)/g,'').replace(/^\\s*[-*]\\s+/gm,'').replace(/[ \\t]+/g,' ').replace(/\\s+-\\s+/g,' - ');value=value.replace(/\\s+(\\d+\\.\\s)/g,'\\n\\n$1').replace(/(\\d+\\.\\s[^\\n]+?)\\s+(\\d{1,2}:\\d{2}\\s*(?:AM|PM))/gi,'$1\\n$2').replace(/\\s+(A strong|Good for|Best for|Great for|Why:|Reason:)/gi,'\\n$1').replace(new RegExp('\\\\s+(Best first tap:)','gi'),'\\n\\n$1').replace(new RegExp('\\\\s+(Directions:)','gi'),'\\n$1');return value.replace(/[ \\t]{2,}/g,' ').replace(/\\n[ \\t]+/g,'\\n').replace(/\\n{3,}/g,'\\n\\n').trim();}
    ...
    function setBrainDebug(info){if(!debug)return;if(!info){debug.classList.remove('is-on');debug.textContent='';return;}var score=Number(info.brainScore||0);var facts=Number(info.learnedFacts||0);var events=Number(info.eventsUsed||0);var source=String(info.source||'Tapzy Brain');debug.innerHTML='<strong>Brain:</strong> '+source+' | Score '+(score?score+'%':'--')+' | Events '+events+' | Learned '+facts+' facts';debug.classList.add('is-on');}
    function pickVoice(){try{var voices=window.speechSynthesis&&window.speechSynthesis.getVoices?window.speechSynthesis.getVoices():[];var english=voices.filter(function(v){return /^en/i.test(v.lang||'');});var preferred=[/samantha.*enhanced/i,/ava.*premium/i,/ava.*enhanced/i,/jenny.*natural/i,/aria.*natural/i,/samantha/i,/ava/i,/victoria/i,/karen/i,/moira/i,/tessa/i,/allison/i,/zira/i,/jenny/i,/aria/i];for(var i=0;i<preferred.length;i++){var match=english.find(function(v){return preferred[i].test(String(v.name||''));});if(match)return match;}return english[0]||voices[0]||null;}catch(_){return null;}}
    function unlockSpeech(){try{if(speechUnlocked||!('speechSynthesis'in window))return;speechUnlocked=true;var u=new SpeechSynthesisUtterance(' ');u.volume=0;u.rate=1;window.speechSynthesis.speak(u);window.setTimeout(function(){try{if(window.speechSynthesis.paused)window.speechSynthesis.resume();}catch(_){}},40);}catch(_){}}
    function browserSpeak(text,attempt){try{if(!('speechSynthesis'in window))return;var clean=String(text||'').replace(new RegExp('https?:\\\\/\\\\/\\\\S+','g'),'').replace(new RegExp('\\\\s+','g'),' ').trim();if(!clean)return;window.speechSynthesis.cancel();if(window.speechSynthesis.paused)window.speechSynthesis.resume();var u=new SpeechSynthesisUtterance(clean.slice(0,900));var voice=pickVoice();if(voice)u.voice=voice;u.lang=voice&&voice.lang?voice.lang:'en-US';u.rate=.91;u.pitch=1.1;u.volume=1;setState('speaking','Speaking');u.onend=function(){settleIdle(700);};u.onerror=function(){if(!attempt){window.setTimeout(function(){browserSpeak(clean,1);},180);return;}settleIdle(700);};window.speechSynthesis.speak(u);window.setTimeout(function(){try{if(window.speechSynthesis.paused)window.speechSynthesis.resume();}catch(_){}},80);}catch(_){settleIdle(700);}}
    function stopTtsAudio(){try{if(ttsAudio){ttsAudio.pause();ttsAudio.src='';ttsAudio=null;}}catch(_){}try{if(ttsUrl){URL.revokeObjectURL(ttsUrl);ttsUrl='';}}catch(_){}}
    function speak(text){var clean=String(text||'').replace(new RegExp('https?:\\\\/\\\\/\\\\S+','g'),'').replace(new RegExp('\\\\s+','g'),' ').trim();if(!clean)return;stopTtsAudio();try{if(window.speechSynthesis)window.speechSynthesis.cancel();}catch(_){}setState('speaking','Speaking');fetch('/api/assistant/tts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:clean.slice(0,1200)})}).then(function(res){if(!res.ok)throw new Error('tts_unavailable');return res.blob();}).then(function(blob){ttsUrl=URL.createObjectURL(blob);ttsAudio=new Audio(ttsUrl);ttsAudio.preload='auto';ttsAudio.onended=function(){stopTtsAudio();settleIdle(700);};ttsAudio.onerror=function(){stopTtsAudio();browserSpeak(clean);};var play=ttsAudio.play();if(play&&play.catch)play.catch(function(){stopTtsAudio();browserSpeak(clean);});}).catch(function(){stopTtsAudio();browserSpeak(clean);});}
    try{if(window.speechSynthesis){window.speechSynthesis.onvoiceschanged=function(){pickVoice();};pickVoice();}}catch(_){}
    function wantsLocation(text){return /\\b(near me|nearby|around me|around here|my area|current location|location|tonight|today|tomorrow|this weekend|weather|forecast|open now|food|restaurant|snack|coffee|dessert|date|concert|event|events|festival|bar|club|nightlife|directions?|navigate|map|where should|what should we do|places?)\\b/i.test(String(text||''));}function getLocation(force){return new Promise(function(resolve){if(geo&&!force)return resolve(geo);if(geoAsked&&!force)return resolve(null);geoAsked=true;if(!navigator.geolocation){setStatus('Location is not supported here.');window.setTimeout(function(){setStatus('');},1600);return resolve(null);}setStatus('Checking location...');navigator.geolocation.getCurrentPosition(function(pos){geo={latitude:pos.coords.latitude,longitude:pos.coords.longitude,accuracy:pos.coords.accuracy||null};setStatus('Location is on.');window.setTimeout(function(){setStatus('');},900);resolve(geo);},function(){geo=null;geoAsked=false;setStatus('Location is off. Allow location for nearby answers.');window.setTimeout(function(){setStatus('');},2200);resolve(null);},{enableHighAccuracy:true,timeout:8000,maximumAge:120000});});}
    function ask(text){text=String(text||'').trim();if(!text||busy)return;unlockSpeech();showPills();busy=true;setBrainDebug(null);add('user',text);input.value='';setState('thinking','Thinking');setStatus('Checking location...');getLocation(true).then(function(loc){if(!loc||!Number.isFinite(loc.latitude)||!Number.isFinite(loc.longitude)){var locationReply='Please allow location first so Tapzy can give nearby events and places.';add('bot',locationReply);speak(locationReply);throw new Error('location_required');}setStatus('Thinking...');return fetch('/api/assistant/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,sessionId:brainSessionId,pageType:root.dataset.pageType||'general',username:root.dataset.username||'User',isAuthPage:root.dataset.authPage==='true',currentPath:location.pathname,currentUrl:location.href,memory:memory,latitude:loc.latitude,longitude:loc.longitude,accuracy:loc.accuracy,timeZone:browserTimeZone})});}).then(function(res){return res.json().catch(function(){return{};});}).then(function(data){var reply=cleanReply((data&&data.reply)||'Tapzy Assistant is temporarily unavailable.',text);add('bot',reply);setBrainDebug(data&&data.brainDebug);speak(reply);}).catch(function(error){if(error&&error.message==='location_required')return;var reply='Tapzy Assistant is temporarily unavailable. Try again in a moment.';add('bot',reply);speak(reply);}).finally(function(){busy=false;setStatus('');if(!window.speechSynthesis||!window.speechSynthesis.speaking)settleIdle(900);});}
    launch&&launch.addEventListener('click',function(e){e&&e.preventDefault&&e.preventDefault();open();});
    window.__tapzyOpenAssistant=function(){var r=document.querySelector('.tz-ai-root[data-tapzy-assistant]');if(!r)return false;var p=r.querySelector('.tz-ai-panel');var i=r.querySelector('.tz-ai-input');if(!p)return false;p.classList.add('is-open');p.dataset.aiState='idle';try{var rr=document.querySelector('.tz-ai-root[data-tapzy-assistant]');if(rr&&rr.__tapzyAskLocation)rr.__tapzyAskLocation(false);}catch(_){}setTimeout(function(){i&&i.focus&&i.focus();},120);return true;};
    ...
    root.__tapzyAskLocation=getLocation;close&&close.addEventListener('click',shut);
