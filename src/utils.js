...
<style data-tapzy-assistant>
  ...
  .tz-ai-prompts{position:relative;z-index:4;display:flex;align-items:center;height:38px;padding:0 22px 6px;gap:10px;overflow-x:auto;overflow-y:hidden;scrollbar-width:none;transition:opacity .32s ease}.tz-ai-prompts.is-idle-hidden{opacity:0;pointer-events:none}.tz-ai-prompts::-webkit-scrollbar{display:none}.tz-ai-chip{display:inline-flex;align-items:center;justify-content:center;min-height:32px;padding:0 14px;border-radius:999px;font-size:10px;font-weight:900;letter-spacing:1px;text-transform:uppercase;background:rgba(10,18,34,.58);border:1px solid rgba(156,214,255,.22);color:#eef7ff;white-space:nowrap;cursor:pointer}
...
(function(){
  ...
  roots.forEach(function(root){
    ...
    var memory=[],geo=null,geoAsked=false,busy=false,stateTimer=null,pillTimer=null;
    ...
    function showPills(){var prompts=root.querySelector('.tz-ai-prompts');if(prompts)prompts.classList.remove('is-idle-hidden');window.clearTimeout(pillTimer);pillTimer=window.setTimeout(function(){if(panel&&panel.classList.contains('is-open')){var p=root.querySelector('.tz-ai-prompts');if(p)p.classList.add('is-idle-hidden');}},20000);}function open(){panel.classList.add('is-open');setState('idle','Idle');showPills();setTimeout(function(){input&&input.focus&&input.focus();},120);if(!geoAsked)getLocation(false);}
