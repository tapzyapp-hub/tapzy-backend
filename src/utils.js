...
function renderTopBar({ currentProfile = null, pageTitle = "Tapzy", pageType = "general" } = {}) {
  ...
  <header class="tz-topbar">
    <div class="tz-topbar-inner">
      <a class="tz-brand" href="/">
        <span class="tz-brand-word">Tapzy</span>
      </a>
    ...
    <div class="tz-menu-panel-inner">
      <div class="tz-menu-head">
        <div>
          ...
          <div class="tz-menu-subtitle">Tapzy Control Center</div>
        </div>
...
function renderShell(title, body, extraHead = "", shellOptions = {}) {
  ...
  const pageTitle = shellOptions.pageTitle || title;
    ...
    <style>
      ...
      *{box-sizing:border-box}
      ...
      .tz-nav-tile{
        ...
        padding:12px 13px;
        border-radius:18px;
        background:linear-gradient(180deg,#1a1b20,#121217);
...
function renderTapzyAssistant({ username = "User", isAuthPage = false, pageType = "general" } = {}) {
  const safeUsername = escapeHtml(username);
  const greeting = isAuthPage ? "Hello. Welcome to Tapzy." : `Welcome back, ${safeUsername}.`;
  ...
  <style>
    .tz-ai-orb{
      ...
      background:
        radial-gradient(circle at 50% 35%, rgba(186,236,255,.95) 0%, rgba(125,214,255,.50) 18%, rgba(79,155,255,.18) 38%, rgba(17,20,34,.96) 68%, rgba(8,8,10,.99) 100%);
      box-shadow:
        0 0 0 1px rgba(255,255,255,.03),
        0 0 16px rgba(111,210,255,.18),
        0 0 34px rgba(111,210,255,.10),
    ...
    .tz-ai-orb::after{
      ...
      inset:0;
      background:
        radial-gradient(circle at 50% 40%, rgba(255,255,255,.24), transparent 26%),
        conic-gradient(from 0deg, rgba(125,214,255,.0), rgba(125,214,255,.24), rgba(255,255,255,.06), rgba(125,214,255,.22), rgba(125,214,255,.0));
      border-radius:999px;
      animation:tzOrbSpin 6.2s linear infinite;
      opacity:.85;
      pointer-events:none;
    }
    ...
    .tz-ai-orb.active{
      box-shadow:
        ...
        0 0 22px rgba(111,210,255,.28),
        0 0 46px rgba(111,210,255,.18),
        0 20px 44px rgba(0,0,0,.46);
      animation:tzOrbPulse 1.05s ease-in-out infinite;
    }
    ...
    .tz-ai-shell.open{display:block}
    ...
    .tz-ai-msg.user{
      align-self:flex-end;
      background:linear-gradient(180deg, #f4fbff, #dff5ff);
      color:#000;
    ...
    .tz-ai-mic.active{
      box-shadow:0 0 0 3px rgba(127,210,255,.14);
      border-color:rgba(127,210,255,.35);
    }
  ...
  <button class="tz-ai-orb" id="tzAiOrb" type="button" aria-label="Open Tapzy Assistant"></button>
  <div class="tz-ai-restore-hint" id="tzAiRestoreHint">Assistant hidden • double tap anywhere to bring it back</div>

  <div class="tz-ai-shell" id="tzAiShell" aria-live="polite">
    <div class="tz-ai-head">
      <div>
        <div class="tz-ai-title">Tapzy Assistant</div>
        <div class="tz-ai-sub">Smart help for search, messages, profile, pair, and events</div>
      </div>
      <button class="tz-ai-close" id="tzAiClose" type="button" aria-label="Close assistant">×</button>
    </div>

    <div class="tz-ai-body">
      <div class="tz-ai-chat" id="tzAiChat"></div>

      <div class="tz-ai-controls">
        <input
          id="tzAiInput"
          class="tz-ai-input"
          type="text"
          placeholder="Ask Tapzy something..."
          autocomplete="off"
          ...
          inputmode="text"
        />
        <button id="tzAiMic" class="tz-ai-mic" type="button" aria-label="Voice input">🎤</button>
        <button id="tzAiSend" class="tz-ai-send" type="button">Send</button>
      </div>
  ...
  <script>
    (function(){
      const orb = document.getElementById("tzAiOrb");
      const shell = document.getElementById("tzAiShell");
      const closeBtn = document.getElementById("tzAiClose");
      const chatEl = document.getElementById("tzAiChat");
      const inputEl = document.getElementById("tzAiInput");
      const sendBtn = document.getElementById("tzAiSend");
      const micBtn = document.getElementById("tzAiMic");
      const restoreHint = document.getElementById("tzAiRestoreHint");
      if (!orb || !shell || !chatEl || !inputEl || !sendBtn || !micBtn) return;

      const username = ${JSON.stringify(username || "User")};
      const greeting = ${JSON.stringify(greeting)};
      const pageType = ${JSON.stringify(pageType)};
      const isAuthPage = ${JSON.stringify(!!isAuthPage)};
      const currentPath = window.location.pathname || "/";
      const currentUrl = window.location.href || "";
      const storageKeyOpen = "tapzy_ai_open";
      const storageKeyHistory = "tapzy_ai_history_" + pageType + "_" + (username || "guest");
      const storageKeyUnlocked = "tapzy_ai_audio_unlocked";
      const storageKeyOrbHidden = "tapzy_ai_orb_hidden";
      ...
      function loadHistory() {
        try {
          const raw = sessionStorage.getItem(storageKeyHistory);
          const parsed = raw ? JSON.parse(raw) : [];
          if (Array.isArray(parsed)) {
            memory = parsed.slice(-12);
          }
        } catch (_e) {
          memory = [];
        }
      ...
      function saveHistory() {
        try {
          sessionStorage.setItem(storageKeyHistory, JSON.stringify(memory.slice(-12)));
        } catch (_e) {}
      }

      function addMessage(role, text, persist = true) {
        const el = document.createElement("div");
        el.className = "tz-ai-msg " + role;
        el.textContent = text;
        chatEl.appendChild(el);
        chatEl.scrollTop = chatEl.scrollHeight;

        if (persist) {
          memory.push({ role: role === "bot" ? "assistant" : "user", content: text });
      ...
      function unlockAudio() {
        ...
        audioUnlocked = true;
        try { sessionStorage.setItem(storageKeyUnlocked, "1"); } catch (_e) {}
      ...
      function speak(text) {
        if (!audioUnlocked || !synth || !text) return;
      ...
      function showRestoreHint() {
        ...
        restoreHint.classList.add("show");
        clearTimeout(hintTimer);
        hintTimer = setTimeout(function(){
          restoreHint.classList.remove("show");
        }, 2200);
      }

      function hideOrb() {
        orb.classList.add("hidden-by-user");
        shell.classList.remove("open");
        try { localStorage.setItem(storageKeyOrbHidden, "1"); } catch (_e) {}
        try { sessionStorage.setItem(storageKeyOpen, "0"); } catch (_e) {}
        showRestoreHint();
      }

      function showOrb() {
        orb.classList.remove("hidden-by-user");
        if (restoreHint) restoreHint.classList.remove("show");
        try { localStorage.setItem(storageKeyOrbHidden, "0"); } catch (_e) {}
      }

      function bot(text, shouldSpeak) {
      ...
      function currentUsernameFromPage() {
        const pathParts = window.location.pathname.split("/").filter(Boolean);
        if (["u","edit","connections","network","qr"].includes(pathParts[0])) {
          return pathParts[1] || "";
        }
        return username ? String(username).toLowerCase().replace(/[^a-z0-9_]/g, "") : "";
      }
      ...
      function openAssistant() {
        ...
        try { sessionStorage.setItem(storageKeyOpen, "1"); } catch (_e) {}
        setTimeout(function(){
      ...
      function localCommand(raw) {
        ...
        const currentUsername = currentUsernameFromPage();

        if (!cmd) return null;

        if (cmd.includes("open search")) {
          return { type: "navigate", text: "Opening search.", href: "/search" };
        }

        if (cmd.includes("open messages") || cmd === "messages" || cmd.includes("message inbox")) {
          return { type: "navigate", text: "Opening messages.", href: "/messages" };
        }
        ...
        if (cmd.includes("open pair") || cmd.includes("pairing") || cmd.includes("tapzy pair")) {
          return { type: "navigate", text: "Opening Tapzy Pair.", href: "/pair" };
        }

        if (cmd.includes("go home") || cmd === "home" || cmd.includes("open home")) {
          return { type: "navigate", text: "Returning home.", href: "/" };
        }

        if ((cmd.includes("open profile") || cmd === "profile") && currentUsername) {
          return { type: "navigate", text: "Opening profile.", href: "/u/" + currentUsername };
        }

        if ((cmd.includes("edit profile") || cmd === "edit") && currentUsername) {
          return { type: "navigate", text: "Opening edit profile.", href: "/edit/" + currentUsername };
        }

        if ((cmd.includes("open connections") || cmd === "connections") && currentUsername) {
          return { type: "navigate", text: "Opening connections.", href: "/connections/" + currentUsername };
        }

        if ((cmd.includes("open network") || cmd === "network") && currentUsername) {
          return { type: "navigate", text: "Opening network.", href: "/network/" + currentUsername };
        }

        if ((cmd.includes("show my qr") || cmd.includes("open qr") || cmd.includes("share my tapzy")) && currentUsername) {
          return { type: "navigate", text: "Opening your Tapzy QR.", href: "/qr/" + currentUsername };
        }

        if (cmd.includes("sign out") || cmd.includes("log out") || cmd.includes("logout")) {
          return { type: "navigate", text: "Signing out.", href: "/logout" };
        }
      ...
      async function askServer(message) {
        try {
          ...
          return "I can help with Tapzy navigation, profiles, messaging, pairing, events, and networking. Try asking something more specific.";
        }
      }

      async function handleInput(raw, opts) {
        const options = Object.assign({ speakReply: false }, opts || {});
        ...
        if (local && local.type === "navigate") {
          bot(local.text, options.speakReply);
          setTimeout(function(){
      ...
      async function sendCurrentInput() {
        ...
        user(text);
        inputEl.value = "";
        await handleInput(text, { speakReply: false });
      }
      ...
      async function startListening() {
        ...
        if (!isSecure) {
          bot("Voice needs HTTPS or localhost.", true);
          return;
        }

        if (!SpeechRecognition) {
          bot("Voice input is not supported on this browser.", true);
          return;
        }

        const permissionOk = await ensureMicrophonePermission();
        if (!permissionOk) {
          bot("Microphone access was blocked. Please allow microphone access and try again.", true);
          return;
        }
        ...
        recognition.onresult = async function(event){
          ...
          if (text) {
            user(text);
            await handleInput(text, { speakReply: true });
          } else {
            bot("I did not catch that.", true);
          }
        };
        ...
        recognition.onerror = function(event){
          ...
          if (err === "not-allowed" || err === "service-not-allowed") {
            bot("Microphone permission was denied.", true);
            return;
          }
          if (err === "no-speech") {
            bot("I did not hear anything. Try again.", true);
            return;
          }
          bot("Voice input error. Tap again and retry.", true);
        };
        ...
        try {
          ...
          micBtn.classList.remove("active");
          orb.classList.remove("active");
          bot("Voice recognition could not start.", true);
        }
      }
      ...
      orb.addEventListener("click", function(){
        unlockAudio();
        showOrb();
        if (shell.classList.contains("open")) closeAssistant();
        else openAssistant();
      });
      ...
      ["touchend","touchcancel","pointerup","pointercancel","mouseleave"].forEach(function(evt){
        orb.addEventListener(evt, function(){
          clearTimeout(longPressTimer);
        }, { passive: true });
      });
      ...
      inputEl.addEventListener("focus", unlockAudio);
      ...
      try {
        if (sessionStorage.getItem(storageKeyUnlocked) === "1") {
          audioUnlocked = true;
        }
      } catch (_e) {}
      ...
      try {
        if (localStorage.getItem(storageKeyOrbHidden) === "1") {
          orb.classList.add("hidden-by-user");
          showRestoreHint();
        }
      } catch (_e) {}
      ...
      try {
        if (sessionStorage.getItem(storageKeyOpen) === "1" && !orb.classList.contains("hidden-by-user")) {
          shell.classList.add("open");
        }
      } catch (_e) {}
