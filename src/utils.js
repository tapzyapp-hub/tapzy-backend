...
<style data-tapzy-assistant>
  ...
  .tz-ai-msg{max-width:88%;min-width:0;margin:0;padding:12px 15px;border-radius:22px;font:520 12.75px/1.46 Inter,"SF Pro Text","Segoe UI",system-ui,sans-serif;letter-spacing:.01px;white-space:pre-wrap;overflow-wrap:anywhere;word-break:normal;background:linear-gradient(180deg,rgba(18,24,38,.9),rgba(9,14,24,.82));border:1px solid rgba(166,205,255,.2);box-shadow:0 18px 42px rgba(0,0,0,.34),inset 0 1px 0 rgba(255,255,255,.09),inset 0 -18px 44px rgba(44,105,190,.05);text-shadow:0 1px 10px rgba(156,206,255,.06);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);-webkit-font-smoothing:antialiased;text-rendering:geometricPrecision}.tz-ai-msg.bot{align-self:flex-start;width:100%;max-width:100%;color:rgba(248,251,255,.97);border-radius:22px}.tz-ai-msg.user{align-self:flex-end;width:auto;max-width:78%;font-weight:700;line-height:1.34;background:linear-gradient(145deg,#2f7bff,#1559e6);border-color:rgba(150,205,255,.24);color:#fff;box-shadow:0 14px 32px rgba(35,105,255,.26)}
...
(function(){
  ...
  roots.forEach(function(root){
    ...
    function speak(text,attempt){try{if(!('speechSynthesis'in window))return;var clean=String(text||'').replace(new RegExp('https?:\\\\/\\\\/\\\\S+','g'),'').replace(new RegExp('\\\\s+','g'),' ').trim();if(!clean)return;window.speechSynthesis.cancel();if(window.speechSynthesis.paused)window.speechSynthesis.resume();var u=new SpeechSynthesisUtterance(clean.slice(0,900));var voice=pickVoice();if(voice)u.voice=voice;u.lang=voice&&voice.lang?voice.lang:'en-US';u.rate=.94;u.pitch=1.07;u.volume=1;setState('speaking','Speaking');u.onend=function(){settleIdle(700);};u.onerror=function(){if(!attempt){window.setTimeout(function(){speak(clean,1);},180);return;}settleIdle(700);};window.speechSynthesis.speak(u);window.setTimeout(function(){try{if(window.speechSynthesis.paused)window.speechSynthesis.resume();}catch(_){}},80);}catch(_){settleIdle(700);}}
