...
<style data-tapzy-assistant>
  ...
  @keyframes tzAiRoomGrid{from{transform:translate3d(0,0,0)}to{transform:translate3d(54px,54px,0)}}@keyframes tzAiFloat{0%,100%{transform:translate(-50%,-50%) translateY(0)}50%{transform:translate(-50%,-50%) translateY(-10px)}}@keyframes tzAiGlow{0%,100%{opacity:.72;transform:scale(.98)}50%{opacity:1;transform:scale(1.06)}}@keyframes tzAiRing{to{transform:rotate(360deg)}}@keyframes tzAiMouthSpeak{0%,100%{transform:translateX(-50%) scaleY(.5)}50%{transform:translateX(-50%) scaleY(1.3)}}@keyframes tzAiFaceSpeak{0%,100%{filter:contrast(1.06) brightness(1)}50%{filter:contrast(1.14) brightness(1.08)}}
  ...
  .tz-ai-stage{position:relative;z-index:3;display:flex;align-items:center;justify-content:center;padding:18px 12px 10px}
  .tz-ai-room-card{position:relative;width:min(92vw,540px);height:min(52vh,560px);min-height:330px;border:1px solid rgba(150,210,255,.18);border-radius:34px;overflow:hidden;background:linear-gradient(180deg,rgba(14,28,48,.42),rgba(0,0,0,.28));box-shadow:0 30px 120px rgba(0,0,0,.72),inset 0 1px 0 rgba(255,255,255,.08);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}
  .tz-ai-room-card::before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 50% 42%,rgba(65,144,255,.18),transparent 30%),linear-gradient(rgba(255,255,255,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.03) 1px,transparent 1px);background-size:100% 100%,42px 42px,42px 42px}
  ...
  .tz-ai-face{position:relative;width:100%;height:100%;border-radius:50%;overflow:hidden;background:#06101f;box-shadow:inset 0 0 46px rgba(0,0,0,.78);z-index:2}.tz-ai-face img{width:100%;height:100%;object-fit:cover;display:block;filter:grayscale(1) contrast(1.18) brightness(.86)}.tz-ai-face::after{content:"";position:absolute;left:64%;top:64%;width:16%;height:4%;border-radius:999px;background:rgba(255,255,255,.48);transform:translateX(-50%) scaleY(.5);opacity:.6}
  .tz-ai-state{position:absolute;left:28px;bottom:28px;z-index:5}.tz-ai-state-pill{display:inline-flex;align-items:center;gap:10px;padding:12px 16px;border-radius:999px;background:rgba(1,5,12,.58);border:1px solid rgba(255,255,255,.12);font-weight:950}.tz-ai-state-dot{width:10px;height:10px;border-radius:50%;background:#3aa0ff;box-shadow:0 0 18px rgba(58,160,255,.85)}
  .tz-ai-body{position:relative;z-index:4;display:flex;flex:1;flex-direction:column;gap:10px;width:100%;max-height:24vh;min-height:110px;overflow:auto;padding:8px 22px;box-sizing:border-box;scrollbar-width:none}.tz-ai-body::-webkit-scrollbar{display:none}
  .tz-ai-msg{max-width:88%;min-width:0;margin:0;padding:12px 14px;border-radius:18px;font-size:14px;line-height:1.42;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;background:rgba(18,22,32,.64);border:1px solid rgba(180,210,255,.11);box-shadow:inset 0 1px 0 rgba(255,255,255,.04);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px)}.tz-ai-msg.bot{align-self:flex-start;color:rgba(255,255,255,.93)}.tz-ai-msg.user{align-self:flex-end;background:linear-gradient(145deg,#2f7bff,#1559e6);border-color:rgba(150,205,255,.24);color:#fff;box-shadow:0 14px 32px rgba(35,105,255,.26)}
  .tz-ai-prompts{position:relative;z-index:4;display:flex;align-items:center;height:40px;padding:0 22px 6px;gap:10px;overflow-x:auto;overflow-y:hidden;scrollbar-width:none}.tz-ai-prompts::-webkit-scrollbar{display:none}.tz-ai-chip{display:inline-flex;align-items:center;justify-content:center;min-height:32px;padding:0 14px;border-radius:999px;font-size:10px;font-weight:900;letter-spacing:1px;text-transform:uppercase;background:rgba(10,18,34,.58);border:1px solid rgba(156,214,255,.22);color:#eef7ff;white-space:nowrap;cursor:pointer}
  ...
  .tz-ai-panel[data-ai-state="listening"] .tz-ai-state-text::after{content:""}.tz-ai-panel[data-ai-state="speaking"] .tz-ai-face::after{animation:tzAiMouthSpeak .72s ease-in-out infinite}.tz-ai-panel[data-ai-state="speaking"] .tz-ai-face{animation:tzAiFaceSpeak 1.8s ease-in-out infinite}
  @media(max-width:520px){.tz-ai-room-card{width:calc(100vw - 24px);height:min(50vh,500px);min-height:330px}.tz-ai-orb{width:min(42vw,160px);top:39%}.tz-ai-body{max-height:23vh;padding-left:20px;padding-right:20px}.tz-ai-status{padding-bottom:calc(86px + env(safe-area-inset-bottom))}}
</style>
<div class="tz-ai-root" data-tapzy-assistant data-username="__USERNAME__" data-page-type="__PAGETYPE__" data-auth-page="__ISAUTH__">
  ...
  <section class="tz-ai-panel" aria-label="Ask Tapzy assistant" role="dialog" aria-modal="false">
    ...
    <main class="tz-ai-stage"><div class="tz-ai-room-card"><div class="tz-ai-orb" aria-hidden="true"><div class="tz-ai-face"><img src="/images/tapzy-identity-digital-face.jpg" alt=""/></div></div><div class="tz-ai-state"><div class="tz-ai-state-pill"><span class="tz-ai-state-dot"></span><span class="tz-ai-state-text">Idle</span></div></div></div></main>
...
(function(){
  ...
  roots.forEach(function(root){
    ...
    function open(){panel.classList.add('is-open');setState('idle','Idle');setTimeout(function(){input&&input.focus&&input.focus();},120);if(!geoAsked)getLocation(false);}
