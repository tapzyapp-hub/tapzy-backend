...
(function(){
  ...
  roots.forEach(function(root){
    ...
    function beautify(text){return String(text||'').replace(/\\*\\*([^*]+)\\*\\*/g,'$1').replace(/__([^_]+)__/g,'$1').replace(new RegExp('\\\\x60([^\\\\x60]+)\\\\x60','g'),'$1').replace(/\\[([^\\]]+)\\]\\([^)]*\\)/g,'$1').replace(/\\(\\[\\]\\(|\\[\\]\\(|\\(\\)/g,'').replace(/^\\s*[-*]\\s+/gm,'').replace(/\\s+-\\s+/g,' - ').replace(new RegExp('\\\\s+(\\\\d+\\\\.\\\\s)','g'),'\\n$1').replace(new RegExp('\\\\s+(Best first tap:)','gi'),'\\n\\n$1').replace(new RegExp('\\\\s+(Directions:)','gi'),'\\n$1').replace(/[ \\t]{2,}/g,' ').replace(/\\n{3,}/g,'\\n\\n').trim();}
