...
function renderTapzyAssistant(options = {}) {
  ...
  .tz-ai-orb{position:absolute;left:50%;top:44%;width:min(70vw,390px);aspect-ratio:1/1;transform:translate(-50%,-50%);display:grid;place-items:center;filter:drop-shadow(0 34px 80px rgba(0,0,0,.72));animation:tzAiFloat 6.4s ease-in-out infinite;z-index:2;}
  .tz-ai-orb::before{content:"";position:absolute;inset:7%;border-radius:50%;background:radial-gradient(circle,rgba(83,163,255,.3),transparent 58%);box-shadow:0 0 56px rgba(48,129,255,.45),0 0 120px rgba(48,129,255,.22);animation:tzAiGlow 4.8s ease-in-out infinite;}
  .tz-ai-orb::after{content:"";position:absolute;inset:2%;border-radius:50%;border:1px solid rgba(147,213,255,.22);box-shadow:inset 0 0 42px rgba(96,171,255,.12);animation:tzAiRing 8s linear infinite;}
  .tz-ai-face{position:relative;width:82%;aspect-ratio:1/1;border-radius:28%;overflow:hidden;background:#050608;transform:perspective(900px) rotateX(0deg) rotateY(-4deg);animation:tzAiFaceBreathe 5.4s ease-in-out infinite;box-shadow:inset 0 0 0 1px rgba(255,255,255,.06),0 0 52px rgba(206,230,255,.17);}
  .tz-ai-face img{position:absolute;inset:-4%;width:108%;height:108%;object-fit:cover;opacity:.88;filter:grayscale(1) brightness(1.08) contrast(1.14);mix-blend-mode:screen;}
  .tz-ai-face::before{content:"";position:absolute;inset:0;background:linear-gradient(rgba(255,255,255,.09) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.07) 1px,transparent 1px);background-size:12px 12px;opacity:.18;animation:tzAiPixelDrift 8s linear infinite;}
